...
replace_once(
    ...
    "function tryStrategicHold(sx,sy){if(!gameStarted||paused||interactionMode)return false;const t=screenToTile(sx,sy);if(!inBounds(t.x,t.y))return false;const clicked=selectableEntityAt(t.x,t.y,false);if(clicked&&clicked.faction===PLAYER)return false;const list=selectedEntities().filter(ownEntity)",
