...
const routeEntrance = (id, roomId, x, z, w = 0.72, d = 0.72, extra = {}) => freeze({
  ...
  style: extra.style ?? "gnawed-wall",
});

export const TUNNEL_FAMILY_PROFILES = freeze({
  "gnawed-wall": freeze({ label: "gnawed wall", material: "wood-plaster", acoustic: "soft", catAccess: false }),
  plumbing: freeze({ label: "plumbing run", material: "pipe-tile", acoustic: "drip", catAccess: false }),
  cabinet: freeze({ label: "cabinet crawlspace", material: "wood-packaging", acoustic: "crumb-rustle", catAccess: false }),
  ventilation: freeze({ label: "ventilation duct", material: "ribbed-metal", acoustic: "metallic", catAccess: false }),
  furniture: freeze({ label: "furniture burrow", material: "fabric-frame", acoustic: "muffled", catAccess: false }),
});

export const SECRET_ROUTE_ENTRANCES = freeze([
  routeEntrance("study-hall-west", "study", -5.03, -9, 0.72, 0.86, { axis: "x", normalX: -1, normalZ: 0, style: "gnawed-wall" }),
  routeEntrance("study-hall-east", "hallway", -4.72, -9, 0.72, 0.86, { axis: "x", normalX: 1, normalZ: 0, style: "gnawed-wall" }),
  routeEntrance("bath-bedroom-west", "bathroom", -1.48, -13, 0.78, 0.72, { axis: "x", normalX: -1, normalZ: 0, style: "plumbing" }),
  routeEntrance("bath-bedroom-east", "bedroom", -1.12, -13, 0.78, 0.72, { axis: "x", normalX: 1, normalZ: 0, style: "gnawed-wall" }),
  routeEntrance("children-bath-west", "children", -5.18, -14.25, 0.78, 0.72, { axis: "x", normalX: -1, normalZ: 0, style: "gnawed-wall" }),
  routeEntrance("children-bath-east", "bathroom", -4.53, -14.25, 0.78, 0.72, { axis: "x", normalX: 1, normalZ: 0, style: "plumbing" }),
  routeEntrance("pantry-cellar-west", "pantry", 7.02, -10.55, 0.72, 0.78, { axis: "x", normalX: -1, normalZ: 0, style: "cabinet" }),
  routeEntrance("pantry-cellar-east", "basement-access", 7.38, -10.55, 0.72, 0.78, { axis: "x", normalX: 1, normalZ: 0, style: "ventilation" }),
  routeEntrance("garage-utility-west", "utility", 18.62, 2.55, 0.72, 0.78, { axis: "x", normalX: -1, normalZ: 0, style: "ventilation" }),
  routeEntrance("garage-utility-east", "garage", 18.98, 2.55, 0.72, 0.78, { axis: "x", normalX: 1, normalZ: 0, style: "ventilation" }),
  routeEntrance("kitchen-wall-hole", "kitchen", 5.75, -6.18, 0.42, 0.42, { axis: "z", normalX: 0, normalZ: 1, style: "cabinet" }),
  routeEntrance("pantry-wall-hole", "pantry", 5.75, -6.58, 0.42, 0.42, { axis: "z", normalX: 0, normalZ: -1, style: "cabinet" }),
  routeEntrance("bedroom-baseboard-hole", "bedroom", 1.86, -13.05, 0.42, 0.42, { axis: "x", normalX: -1, normalZ: 0, style: "gnawed-wall" }),
  routeEntrance("bedroom-closet-gap", "bedroom", 1.86, -15.15, 0.42, 0.42, { axis: "x", normalX: -1, normalZ: 0, style: "furniture" }),
  routeEntrance("dining-sideboard-hole", "dining", 5.72, 9.38, 0.42, 0.42, { axis: "x", normalX: -1, normalZ: 0, style: "cabinet" }),
  routeEntrance("hall-console-hole", "hallway", -3.85, -8.12, 0.42, 0.42, { axis: "x", normalX: 1, normalZ: 0, style: "gnawed-wall" }),
  routeEntrance("refrigerator-gap", "kitchen", 7.28, 3.65, 0.42, 0.42, { axis: "x", normalX: 1, normalZ: 0, style: "ventilation" }),
  routeEntrance("utility-vent-hole", "utility", 15.28, 5.25, 0.42, 0.42, { axis: "x", normalX: 1, normalZ: 0, style: "ventilation" }),
  routeEntrance("bathtub-gap", "bathroom", -1.9, -13.46, 0.42, 0.42, { axis: "z", normalX: 0, normalZ: 1, style: "plumbing" }),
  routeEntrance("bathroom-wall-hole", "bathroom", -4.43, -14.55, 0.42, 0.42, { axis: "x", normalX: 1, normalZ: 0, style: "plumbing" }),
...
const secretRoute = (id, label, a, b, discoveryNight, family, options = {}) => freeze({
  id,
  label,
  a,
  b,
  ...
  crawlHeight: options.crawlHeight ?? 0.24,
  mouseOnly: true,
  oneWay: options.oneWay ?? false,
  alcove: options.alcove ? freeze({ ...options.alcove }) : null,
});

export const SECRET_ROUTES = freeze([
  secretRoute("couch-burrow", "couch burrow", "couch-left-hole", "couch-right-hole", 1, "furniture", { noise: 0.05, speed: 1, duration: 1.05 }),
  secretRoute("kitchen-pantry-run", "lower-cabinet service tunnel", "kitchen-wall-hole", "pantry-wall-hole", 4, "cabinet", { noise: 0.08, speed: 1, duration: 1.28, alcove: { side: -1, kind: "crumb-cache" } }),
  secretRoute("dining-hallway-run", "sideboard trim passage", "dining-sideboard-hole", "hall-console-hole", 5, "cabinet", { noise: 0.1, speed: 0.9, duration: 2.25 }),
  secretRoute("bathtub-wall-run", "bathtub pipe run", "bathtub-gap", "bathroom-wall-hole", 6, "plumbing", { noise: 0.08, speed: 0.92, duration: 1.42 }),
  secretRoute("study-wall-gap", "study baseboard passage", "study-hall-west", "study-hall-east", 6, "gnawed-wall", { noise: 0.08, speed: 1, duration: 1.18, alcove: { side: 1, kind: "paper-nest" } }),
  secretRoute("bedroom-closet-run", "closet-wall burrow", "bedroom-baseboard-hole", "bedroom-closet-gap", 7, "furniture", { noise: 0.06, speed: 0.94, duration: 1.48, alcove: { side: -1, kind: "button-cache" } }),
  secretRoute("bathroom-pipe-gap", "bathroom-bedroom pipe gap", "bath-bedroom-west", "bath-bedroom-east", 7, "plumbing", { noise: 0.12, speed: 0.88, duration: 1.32 }),
  secretRoute("refrigerator-utility-run", "refrigerator service chase", "refrigerator-gap", "utility-vent-hole", 8, "ventilation", { noise: 0.16, speed: 0.84, duration: 2.45 }),
  secretRoute("toy-baseboard-hole", "toy-room wall tunnel", "children-bath-west", "children-bath-east", 8, "gnawed-wall", { noise: 0.34, speed: 0.82, duration: 1.72, alcove: { side: 1, kind: "tiny-lookout" } }),
  secretRoute("pantry-cellar-chute", "pantry cellar service chute", "pantry-cellar-west", "pantry-cellar-east", 10, "cabinet", { noise: 0.45, speed: 0.76, duration: 1.55 }),
  secretRoute("garage-utility-vent", "garage utility ventilation duct", "garage-utility-west", "garage-utility-east", 11, "ventilation", { noise: 0.6, speed: 0.72, duration: 1.68, width: 0.32 }),
]);
