...
export function installRpgItems({getRpg,getVerses,conceptKeys,conceptLabel,matchWeightedConcepts,matchVerseConcept,refKey,displayRef,refresh}){
  ...
  function installUi(){
    ...
    style.textContent=`
      ...
      .rpg-item-rarity{font-size:.62rem;text-transform:uppercase;letter-spacing:.1em;color:#c4b5fd;font-weight:1000;margin:3px 0 6px}.rpg-item-desc{font-size:.76rem;line-height:1.35;color:#a5b4cc}
      .rpg-hint-reveal{margin:7px 0 0;padding:7px 9px;border:1px solid #854d0e;border-radius:10px;background:#2a1d08;color:#fde68a;font-size:.76rem;font-weight:850}
  ...
  function onSpawn(){installUi();hideHint();hideLoot();closePanel();render();}
  ...
  function useItem(id){
    ...
    if(item.kind==="heal"){const healed=heal(item.amount);if(!healed){used=false;message="You are already at full health.";}else message=`${item.name} restores ${healed} health.`;}
    else if(item.kind==="shield"){rpg.itemShields=Math.min(3,(rpg.itemShields||0)+1);message="Armor of Faith is ready. Your next failed verse will not cost health.";}
    else if(item.kind==="refresh"){rpg.used.clear();message="Scroll of Recall refreshes every verse used during this run.";}
    else if(item.kind==="great-recall"){rpg.used.clear();const healed=heal(.35)||0;rpg.itemShields=Math.min(3,(rpg.itemShields||0)+1);message=`Great Scroll of Recall refreshes your verses, restores ${healed} health, and grants a shield charge.`;}
    else if(item.kind==="category"){const added=bonusConcepts(item.count||1,item.multiplier||1);if(!added.length){used=false;message="No additional Scripture categories are available for this fight.";}else message=`New weakness${added.length>1?"es":""}: ${added.map(conceptLabel).join(" + ")}. You may now attack with verses from ${added.length>1?"those categories":"that category"}.`;}
    else{const verse=pickHintVerse();if(!verse){used=false;message="No unused matching verse is available for a hint.";}else if(item.kind==="hint-words")message=`Word fragment — “${fragment(verse.text,7)}” — no book or reference revealed.`;else if(item.kind==="hint-book")message=`Book Lantern: one unused valid verse is in ${verse.book}.`;else if(item.kind==="hint-chapter")message=`Chapter Map: one unused valid verse is in ${verse.book} ${verse.chapter}.`;else if(item.kind==="hint-reference")message=`Reference Compass: ${displayRef(verse)} can damage this enemy.`;else{used=false;message="That item cannot be used right now.";}}
    if(used)removeOne(id);showHint(message);render();if(typeof refresh==="function")refresh();renderPanel();
  }
