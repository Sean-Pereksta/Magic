...
/* High-level second render pass: parallax silhouettes, local lights, boss arenas, weather and a corrected simulation order. */
const _baseAtmosphere=drawAtmosphere;
drawAtmosphere=function(room,pal){
 _baseAtmosphere(room,pal);ctx.save();
 const horizon=H*.24;
 ctx.globalAlpha=.18;ctx.fillStyle=colorAlpha(pal.accent,.14);
 for(let layer=0;layer<3;layer++){
  ctx.beginPath();ctx.moveTo(0,horizon+70+layer*28);for(let x=0;x<=W+80;x+=80){const y=horizon+layer*36+Math.sin(x*.008+room.x*.7+layer)*22+Math.sin(x*.021+room.y)*10;ctx.lineTo(x,y)}ctx.lineTo(W,H*.48);ctx.lineTo(0,H*.48);ctx.closePath();ctx.fill();ctx.globalAlpha*=.6
 }
 ...
 if(room.biome==='volcanic'){ctx.globalAlpha=.16;ctx.fillStyle='#ff5736';for(let i=0;i<7;i++){const x=(i*211+room.x*77)%W;const g=ctx.createRadialGradient(x,horizon+30,0,x,horizon+30,70);g.addColorStop(0,'rgba(255,80,42,.2)');g.addColorStop(1,'rgba(255,80,42,0)');ctx.fillStyle=g;ctx.fillRect(x-80,horizon-50,160,160)}}
 ctx.restore();
};
const _baseRoomLight=drawRoomLight;
drawRoomLight=function(room,pal){
 _baseRoomLight(room,pal);ctx.save();ctx.globalCompositeOperation='screen';
 const lights=[];if(game.player){const a=game.player.armorGear?.aura;if(a)lights.push({x:game.player.x,y:game.player.y,c:a==='ember'?'#ff7448':a==='frost'?'#9eeaff':a==='leaf'?'#6ddd7e':a==='void'?'#b86cff':a==='sun'?'#ffd365':'#76e5ff',r:95})}
 ...
 for(const l of lights.slice(0,30)){const s=worldToScreen(l.x,l.y),g=ctx.createRadialGradient(s.x,s.y,0,s.x,s.y,l.r);g.addColorStop(0,colorAlpha(l.c,.1));g.addColorStop(1,colorAlpha(l.c,0));ctx.fillStyle=g;ctx.fillRect(s.x-l.r,s.y-l.r,l.r*2,l.r*2)}ctx.restore();
 if(game.enemies.some(e=>e.boss))drawBossArena();drawForegroundWeather(room,pal)
};
...
const _baseDrawProp=drawProp;
drawProp=function(p,biome){const s=worldToScreen(p.x,p.y);shadowAt(p.x,p.y,18*p.scale,.16);ctx.save();ctx.translate(-s.x,-s.y);ctx.globalAlpha=0;ctx.restore();_baseDrawProp(p,biome)};
