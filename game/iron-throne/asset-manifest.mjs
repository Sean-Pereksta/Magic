...
const levels = (id, b) => b.levels.map(l => `buildings/${id}_${l.level}.png`);
...
export const ART = {
  structures: Object.fromEntries(Object.entries(BUILDINGS).map(([id,b]) => [id, Object.fromEntries(b.levels.map(l => [l.level, ironThronesAsset(`buildings/${id}_${l.level}.png`)]))])),
...
for (const [id,b] of Object.entries(BUILDINGS)) for (const l of b.levels) fallbacks.set(ART.structures[id][l.level],local(`structures/${id}_${l.level}`));
