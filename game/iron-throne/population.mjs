...
export function populationCapacity(s, owner) {
  return Object.values(s.tiles).filter(t=>t.owner===owner).reduce((n,t)=>n+(t.building==='city'?150:t.building==='town'?80:0)+buildingLevel(t,'farm')*8,0);
}
export function calculatePopulationChange(s, owner, income) {
  ...
  if(k.population>=capacity)blockers.push('Population capacity reached. Add settlements or farm levels.');
