...
import { emotionalEvent, personalContext, personalOpening, personalWillingness } from './emotions.mjs';
import { MARRIAGE_FIELDS, normalizeMarriageTerms, evaluateMarriage, commitMarriage, marriageContext, marriageReply, marriageSupport, resolveMarriages, strainMarriage } from './marriage.mjs';
...
export function validateIntent(value) {
  ...
  if(value.type==='MARRIAGE'){if(value.tradeKind!==undefined)return null;const terms=normalizeMarriageTerms(value);if(!terms)return null;Object.assign(i,terms);}
  else if(MARRIAGE_FIELDS.some(key=>value[key]!==undefined))return null;
...
export function describeIntent(i) {
  ...
  return `${LABELS[i.type]}${i.targetId ? ` · ${i.targetId}` : ''}${i.giveAmount ? ` · Proposer gives ${i.giveAmount} ${i.giveResource}${timing}` : ''}${i.receiveAmount ? ` · Proposer receives ${i.receiveAmount} ${i.receiveResource}${i.type === 'LOAN' ? ' on the deadline' : i.type === 'RECURRING' ? ' each turn' : ' now'}` : ''} · ${duration}${i.conditionHouseId || i.type === 'GUARANTEE' ? ` · only if ${i.conditionHouseId || i.targetId} attacks this House` : ''}`;
}
export function dealFactors(s, rulerId, i, actorHouseId = PLAYER) {
  const r = relation(s, rulerId, actorHouseId), k = kingdom(s, rulerId), military = borderThreat(s, rulerId, actorHouseId), economic = economicRelationship(s, rulerId, actorHouseId);
  const factors = [];
  ...
  if (military.score >= 20) factors.push('Your armies are concentrated near our frontier.');
  ...
  if (economic.majorPartner) factors.push('Your House supplies an important share of our imports.');
  if (military.sharedEnemies.length) factors.push('We face a shared enemy.');
  if ((r.reliability ?? 50) > 60) factors.push('Your House has honored its previous promises.');
...
export function evaluateDeal(s, rulerId, raw, actorHouseId = PLAYER, { consentingHuman = false } = {}) {
  ...
  if (isPlayerPromise(i)) {
    const error = playerPromiseCheck(s, rulerId, i, actorHouseId);
  ...
  if (['EXCHANGE', 'TRADE', 'RECURRING', 'LOAN'].includes(i.type) && tradeBlocked(s, actorHouseId, rulerId)) return reject('An active embargo prevents this economic agreement.');
  if (['WAR', 'BETRAY'].includes(i.type)) {
    if (atWar(s, actorHouseId, rulerId)) return reject('You are already at war.');
    return { status: 'accept', reason: treaty(s, actorHouseId, rulerId) ? 'This breaks your treaties and damages your reputation with every house.' : 'Hostilities open borders for invasion. Armies still need marching orders.', intent };
  }
  if (i.type === 'PEACE' && !atWar(s, actorHouseId, rulerId)) return reject('You are already at peace.');
  ...
  if (treatyType && treatyType!=='recurring' && treaty(s, actorHouseId, rulerId, treatyType)) return reject('This agreement is already active.');
  if(surrender){
    if(!surrender.eligible||!consentingHuman&&!surrender.willing)return reject(surrender.reason);
    return {status:'accept',reason:surrender.reason,intent,factors,capitulation:true};
  }
  if (i.type === 'AID') {
  ...
  if (i.type === 'EMBARGO') {
    if (!alive(s, i.targetId) || [actorHouseId, rulerId].includes(i.targetId)) return reject('Choose a third surviving House to embargo.');
    if (!consentingHuman && treaty(s, rulerId, i.targetId, 'alliance')) return reject('We will not embargo our ally.');
  ...
  if (i.type === 'TERRITORY') {
    ...
    if (!Object.values(s.tiles).some(n => n.owner === actorHouseId && distance(n, t) === 1)) return reject('The province must adjoin your land.');
  }
  if (i.type === 'JOINT_WAR') {
    if (!kingdom(s, i.targetId) || [actorHouseId, rulerId].includes(i.targetId) || !alive(s, i.targetId)) return reject('Choose a third surviving house.');
    if (treaty(s, rulerId, i.targetId) || treaty(s, actorHouseId, i.targetId)) return reject('Existing treaties forbid this joint invasion.');
  ...
  if (i.type === 'WITHDRAW' && !armiesOf(s, rulerId).some(a => s.tiles[a.tile].owner === actorHouseId)) return reject('There are no troops of this house in your territory.');
  ...
  if (i.type === 'PEACE') {utility += (playerPower - rulerPower) * .7;if(war?.score>=55)utility=Math.max(utility,threshold);}
  if(i.type==='TRIBUTE'&&war?.score>=55)utility+=(war.score-30)*2;
...
export function commitDeal(s, rulerId, raw, actorHouseId = PLAYER, options = {}) {
  if (isHumanHouse(s, rulerId) && s.controllers && !options.consentingHuman && !['WAR','BETRAY'].includes(raw?.type)) return {ok:false,error:'The receiving human ruler must accept this proposal.'};
  ...
  if (i.type === 'RECURRING') { const route=tradeRoute(s,actorHouseId,rulerId);if(i.tradeKind==='preferential')route.fee=0;Object.assign(s.treaties.at(-1), { intent: i, payer: actorHouseId, lastPaid: s.turn, anchors: contractAnchors(s,actorHouseId,rulerId), legacyRoute:false, routeStatus:route.status });for(const party of [player,k])pay(party,{gold:route.fee}); }
  if (i.type === 'EMBARGO') {
    addTreaty(s, actorHouseId, rulerId, 'embargo', i.duration); s.treaties.at(-1).targetId = i.targetId;
    s.treaties = s.treaties.filter(t => !(['trade', 'recurring'].includes(t.type) && t.parties.includes(i.targetId) && t.parties.some(id => [actorHouseId, rulerId].includes(id))));
    changeRelation(s, i.targetId, actorHouseId, { opinion: -15, grievance: 12 }, 'A ruler organized a trade embargo.');
  }
...
export function deliverPledge(s, id, actorHouseId = PLAYER) {
  ...
  recordTrade(s, actorHouseId, p.creditor, p.intent.giveResource, p.intent.giveAmount, 'pledge');
...
function finishPledge(s, p, fulfilled) {
  ...
  const militaryPromise=['JOINT_WAR','DEFEND','PLEDGE_WAR','PLEDGE_ATTACK','PLEDGE_DEFEND','GUARANTEE'].includes(p.intent.type);
  emotionalEvent(s,p.creditor,p.debtor,fulfilled?'promise':militaryPromise?'betrayal':'abandonment',{key:p.id,scale:militaryPromise?1.5:p.intent.type==='PROMISE'?Math.min(1,p.intent.giveAmount/60):.7,text:`${debtor.name} ${p.status} the promise: ${LABELS[p.intent.type]}.`});
  if(!fulfilled)strainMarriage(s,p.creditor,p.debtor,'Failed a sworn obligation to the House joined to our family.',{key:p.id});
...
export function verifyPledges(s,{operationsOnly=false,skipOperations=false}={}) {
  for (const p of s.pledges.filter(p => p.status === 'pending'&&(!operationsOnly||p.operationId)&&(!skipOperations||!p.operationId))) {
    ...
    if (p.status === 'pending' && isHumanHouse(s, p.debtor) && p.deadline - s.turn <= 2 && p.triggered !== false) {
      contact(s, p.creditor, `reminder-${p.id}`, `${p.deadline - s.turn} turns remain on your word: ${LABELS[i.type]}. Our council is watching for action.`, 2, p.debtor);
...
export function resolveRecurringTrade(s) {
  for (const t of s.treaties.filter(t => t.type === 'recurring' && t.expires > s.turn && t.lastPaid < s.turn)) {
    ...
    if (!canAfford(payer, give) || !canAfford(receiver, receive)) {
      ...
      for (const [ruler, actor] of [[receiver.id,payer.id],[payer.id,receiver.id]]) if (isHumanHouse(s,actor)) contact(s, ruler, `trade-${t.id}`, 'The agreed shipment could not be made. Our trade agreement has ended; we must negotiate terms our treasuries can sustain.', 100000, actor);
...
export function makeContext(s, rulerId, message, { proposal = null, event = null, actorHouseId = PLAYER } = {}) {
  ...
  const context = {
    ...
    world: {
      ...
      militaryRelationship: { ...military, armyOwner:actorHouseId,territoryOwner:rulerId,nearby: military.nearby.slice(0, 12) },
...
export function acceptRulerMemories(s, rulerId, response, actorHouseId = PLAYER) {
  ...
  if (response.relationshipSummary) {
    const k=kingdom(s,rulerId);
    if(s.controllers){k.conversationSummaries||={};k.conversationSummaries[actorHouseId]=response.relationshipSummary;}
    else k.conversationSummary=response.relationshipSummary;
  }
  for (const text of (response.memoryCandidates || []).slice(0, 2)) {
    remember(s, rulerId, `Council interpretation (unverified): ${text}`, 3);
    Object.assign(kingdom(s, rulerId).memories.at(-1), { subject: actorHouseId, kind: 'interpretation', verified: false });
  }
}
export function scriptedReply(s, rulerId, message, options = {}) {
  ...
  if (options.proposal) {
    ...
    const opening = warOpening(warPosition) || (k.greed > .8 ? 'Let us speak of terms that profit both courts.' : k.honor > .8 ? 'I weigh the obligations as carefully as the payment.' : 'There is a price to committing my House.');
  ...
  if(warPosition&&warOpening(warPosition)){
    const type=/vassal|allegiance|capitulat|bend the knee|submit to/i.test(message)?'VASSALAGE':/peace|truce|terms|surrender/i.test(message)?'PEACE':null;
    const intent=type?validateIntent({type,duration:10}):null;
    const verdict=intent?disclosedDeal(source,rulerId,intent,actorHouseId):null;
    return {reply:`${warOpening(warPosition)} ${verdict?.reason||'Put serious terms before me.'}`,tone:'guarded',intents:intent?[intent]:[]};
  }
  ...
  if (/\b(?:spy|spies|espionage|intelligence)\b/.test(text) && !/allian|peace|truce|trade|gift|aid|declar.*war|attack you/.test(text)) {
    const incident=(s.intelligence?.incidents||[]).filter(i=>[i.owner,i.actor].includes(rulerId)&&[i.owner,i.actor].includes(actorHouseId)).at(-1);
  ...
  if (/\b(?:plans?|schemes?|intentions?|priority|priorities)\b/.test(text) && !/allian|peace|truce|trade|gift|aid|declar.*war|attack you/.test(text)) {
    const known=visiblePlans(s,actorHouseId,rulerId)[0];
