...
import { CAPITAL_NAMES, capitalSeparation, foundingCheck, foundingOutlook, startingFootprint } from './founding.mjs';
const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
export function foundingPanel(s,owner,selected) {
  const founded=s.founding.houses[owner].founded;
  if(founded)return `<h2>Your kingdom is founded</h2><p>${esc(CAPITAL_NAMES[owner])} · ${esc(s.founding.houses[owner].capital)}</p><p>Fog of war has begun. Your troops and settlements reveal nearby land. Waiting for the other human rulers to confirm their capitals.</p>`;
  const outlook=foundingOutlook(s,selected);
  ...
  return `<span class="eyebrow">${esc(MAP_PROFILES[s.mapProfile]?.name)} · SEED ${s.seed}</span><h2>Found your kingdom</h2>
    ...
    ${!founded?`<button class="primary full" data-found-city="${esc(selected)}" data-site-valid="${!error}" ${error?'disabled':''}>FOUND CITY</button><p role="status" class="${error?'negative':'fine'}">${esc(error||'Your full starting package fits naturally within 4 hexes.')}</p>`:`<p class="fine">${esc(CAPITAL_NAMES[owner])} · ${esc(s.founding.houses[owner].capital)}</p>`}
    <p class="fine">Starting territory reaches up to 4 hexes across usable land. Capitals must be at least <b>${capitalSeparation(s)} hexes apart</b>. Shaded red tiles are too close to an established capital. AI Houses choose after the human rulers.</p>
    ...
    <div class="section-label">THE ${s.kingdoms.length} HOUSES</div>${s.kingdoms.map(h=>`<p class="fine">${h.sigil} ${h.name} · ${s.founding.houses[h.id].founded?`${CAPITAL_NAMES[h.id]} founded`:'Choosing a home'}</p>`).join('')}`;
}
