  ...
  for(const width of [1280,390]) {
    ...
    const result=await page.evaluate(async()=>{
      ...
      for(let edge=0;edge<6;edge++) {
        ...
        art.draw(c,{coastMask:0,riverMask:1<<edge,mouthMask:0,hasRiver:true,ground:'plains'},0,0);
        art.draw(c,{coastMask:0,riverMask:1<<oppositeEdge(edge),mouthMask:0,hasRiver:true,ground:'plains'},dx,dy);
      ...
      for(let edge=0;edge<6;edge++) {
        ...
        art.draw(c,{coastMask:1<<edge,riverMask:(1<<edge)|(1<<oppositeEdge(edge)),mouthMask:1<<edge,hasRiver:true,ground:'plains'},0,0);
        const port=edgePort(edge),length=Math.hypot(...port);
      ...
      for(let coastMask=0;coastMask<64;coastMask++) for(let riverMask=0;riverMask<6;riverMask++) art.draw(c,{coastMask,riverMask,hasRiver:!!riverMask,mouthMask:0,ground:'plains'},0,0);
    ...
    console.log(`PASS ${width}px: campaign rendering, reciprocal river pixels, six river mouths, all shoreline masks, bounded cache, map selection`);
    await page.close();
  }
