...
test('civilian ship launch failure preserves the queue and successful retries create one vessel',()=>{
  const h=lab();build(h,'freighter');h.run('state.originalLaunch=launchBuiltShip;launchBuiltShip=()=>null;tickBuildQueue(state.p,0)');assert.equal(h.run('state.p.buildQueue.length'),1);assert.equal(h.run('state.q.stxAction.state'),'RETRYING');h.run('launchBuiltShip=state.originalLaunch;stxActionFinalizeShip(state.p,state.q);stxActionFinalizeShip(state.p,state.q)');assert.equal(h.run('state.p.buildQueue.length'),0);assert.equal(h.run('state.ships.length'),1);
});
test('legacy combat construction delivers reserves without changing an existing fleet',()=>{
  const h=lab();h.run('state.oldFleet=registerFleet(0,state.p,70,"fleet");state.beforeVessels=state.p.reserveVessels');build(h);h.run('tickBuildQueue(state.p,0);stxActionFinalizeShip(state.p,state.q)');assert.equal(h.run('state.fleets.length'),1);assert.equal(h.run('state.oldFleet.strength'),70);assert.equal(h.run('state.p.reserveVessels'),h.run('state.beforeVessels+15'));assert.equal(h.run('state.p.buildQueue.length'),0);
});
test('commissioned patrols and civilian ships remain physically stationed',()=>{
  const h=lab();h.run('chooseShipTarget=()=>null;state.p.reserveVessels=6;state.p.stock.trained=.01;stxLPMobilize(state.p,6,"patrol")');assert.equal(h.run('state.fleets[0].location'),h.run('state.p.id'));build(h,'freighter');h.run('tickBuildQueue(state.p,0);tickShips(100)');assert.equal(h.run('state.ships.length'),1);assert.equal(h.run('state.ships[0].stxStationed'),true);assert.equal(h.run('state.ships[0].progress'),0);
});
