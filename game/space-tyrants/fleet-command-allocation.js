...
function stxFCARedirectTransit(f,s,destination,kind,meta={}){
  const p=destination.planet,origin=state.planets.find(x=>x.id===s.from)||state.planets.find(x=>x.id===f.homePort)||playerWorlds()[0];if(!p||!origin)return false;const strength=Math.max(.05,Number(s.strength)||Number(f.strength)||.05),label=stxFCAOrderLabel(kind,destination);stxFCAClearShipMission(s);s.type="fleet";s.from=origin.id;s.to=p.id;s.startX=Number.isFinite(s.x)?s.x:origin.x;s.startY=Number.isFinite(s.y)?s.y:origin.y;s.progress=0;s.distance=Math.max(1,Math.hypot((Number.isFinite(s.x)?s.x:origin.x)-p.x,(Number.isFinite(s.y)?s.y:origin.y)-p.y));s.speed=Math.max(60,Number(s.speed)||86);s.owner=0;s.strength=strength;s.fleetId=f.id;s.vesselName=f.name;s.status=label;s.stxFleetOrderKind=kind;s.servicePurpose=kind==="invade"?"planetary invasion":"fleet concentration";if(destination.facility)s.stationTargetId=destination.facility.id;Object.assign(s,meta);f.location=null;f.strength=strength;f.readiness="inbound";f.status=label;return true;
}
...
function stxFCADispatchFleet(f,destination,kind,meta={}){
  ...
  const oldDock=f.dockedAt,status=battle?`Disengaging from Battle of ${battle.planet?.name||"Unknown"} → ${label}`:label;f.location=null;f.readiness="inbound";f.status=status;let ship=createShip("fleet",source,target,0,extra);if(!ship)return false;if(oldDock&&f.dockedAt&&typeof stxOLRemoveDocking==="function")stxOLRemoveDocking(f);f.status=status;ship.status=status;stxFCACommitted.add(f.id);return true;
}
