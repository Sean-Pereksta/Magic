...
test('raw-only factory request charges its grant without a hidden authorization fee',()=>{
  const h=lab();h.run(`empire(0).credits=40;state.proposals=[{id:'g',kind:'governor',to:0,planetId:state.p.id,project:'factory',cost:40,status:'pending',title:'Factory'}]`);assert.equal(h.run('stxTXEnqueue("proposal","g")'),false);assert.equal(h.run('stxTXAct("proposal","g","accept")'),true);near(h.run('empire(0).credits'),0);
});
