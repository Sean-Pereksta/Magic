...
test('mobilization consumes selected local hulls and personnel with credits as the only fee',()=>{
 const h=lab();h.run('state.credits=empire(0).credits;state.c=state.p.stock.components;state.e=state.p.stock.equipment;state.f=stxLPMobilize(state.p,20,"fleet","Cerberus")');assert.ok(h.run('state.f'));near(h.run('state.p.reserveVessels'),40);near(h.run('state.p.stock.trained'),.046);near(h.run('empire(0).credits'),h.run('state.credits-40'));near(h.run('state.p.stock.components'),h.run('state.c'));near(h.run('state.p.stock.equipment'),h.run('state.e'));assert.equal(h.run('state.f.location'),h.run('state.p.id'));assert.equal(h.run('state.f.vesselCount'),20);assert.equal(h.run('state.f.mobilizationSources[0].planetId'),h.run('state.p.id'));assert.equal(h.run('state.f.name'),'Cerberus');assert.equal(h.run('state.f.commissionCredits'),40);
});
test('insufficient crew, invalid counts, missing shipyards, and insufficient credits leave reserves untouched',()=>{
 ...
 h.run('state.p.stock.trained=.05;empire(0).credits=29;state.before=JSON.stringify([state.p.reserveVessels,state.p.stock.trained,empire(0).credits]);');assert.equal(h.run('stxLPMobilize(state.p,15)'),false);assert.equal(h.run('JSON.stringify([state.p.reserveVessels,state.p.stock.trained,empire(0).credits])'),h.run('state.before'));
});
test('shipyard build spends raw resources only and adds one reserve hull',()=>{
 const h=lab();h.run('state.before={credits:empire(0).credits,c:state.p.stock.components,e:state.p.stock.equipment,iron:state.p.stock.iron,titanium:state.p.stock.titanium,helium:state.p.stock.helium,rare:state.p.stock.rare,v:state.p.reserveVessels}');assert.equal(h.run('stxLPRush(state.p)'),true);near(h.run('empire(0).credits'),h.run('state.before.credits'));near(h.run('state.p.stock.components'),h.run('state.before.c'));near(h.run('state.p.stock.equipment'),h.run('state.before.e'));near(h.run('state.p.stock.iron'),h.run('state.before.iron-4'));near(h.run('state.p.stock.titanium'),h.run('state.before.titanium-3'));near(h.run('state.p.stock.helium'),h.run('state.before.helium-2'));near(h.run('state.p.stock.rare'),h.run('state.before.rare-1'));near(h.run('state.p.reserveVessels'),h.run('state.before.v+1'));assert.equal(h.run('state.fleets.length'),0);
 h.run('state.p.infra.shipyard=0');assert.equal(h.run('stxLPRush(state.p)'),false);
});
...
test('personnel transit moves crew physically while reserve ships stay on the source planet',()=>{
 const h=lab();destination(h);h.run('state.credits=empire(0).credits;state.sourceShips=state.p.reserveVessels');assert.equal(h.run('stxLPTransfer(state.p,state.dest,15)'),true);assert.equal(h.run('stxLPMobilize(state.dest,15)'),false);near(h.run('state.p.reserveVessels'),h.run('state.sourceShips'));near(h.run('state.p.stock.trained'),.047);near(h.run('state.dest.reserveVessels'),0);near(h.run('empire(0).credits'),h.run('state.credits-3'));assert.equal(h.run('state.ships[0].cargo.militaryVessels||0'),0);near(h.run('state.ships[0].cargo.trained'),.003);h.run('state.s=state.ships[0];tickShips(100)');near(h.run('state.dest.stock.trained'),.003);near(h.run('state.dest.reserveVessels'),0);h.run('state.dest.reserveVessels=15');assert.ok(h.run('stxLPMobilize(state.dest,15)'));h.run('stxActionArrive(state.s,()=>arriveShip(state.s,state.dest))');near(h.run('state.dest.reserveVessels'),0);
});
test('failed personnel dispatch preserves crew and credits, while distant owned planets remain valid destinations',()=>{
 const h=lab();destination(h);h.run('state.originalCreate=createShip;state.before=JSON.stringify([state.p.reserveVessels,state.p.stock.trained,empire(0).credits]);createShip=()=>null');assert.equal(h.run('stxLPTransfer(state.p,state.dest,15)'),false);assert.equal(h.run('JSON.stringify([state.p.reserveVessels,state.p.stock.trained,empire(0).credits])'),h.run('state.before'));h.run('createShip=state.originalCreate;state.dest.x=state.p.x+3000');assert.equal(h.run('stxLPTransfer(state.p,state.dest,15)'),true);near(h.run('state.p.reserveVessels'),60);near(h.run('state.p.stock.trained'),.047);
});
test('captured transit destination returns personnel without granting them to the captor',()=>{
 const h=lab();destination(h);h.run('stxLPTransfer(state.p,state.dest,15);state.dest.owner=1;tickShips(100)');near(h.run('state.dest.stock.trained'),0);near(h.run('state.p.reserveVessels'),60);h.run('tickShips(100)');near(h.run('state.p.reserveVessels'),60);near(h.run('state.p.stock.trained'),.05);
});
test('save round trip preserves parallel progress, modes, reserves and fleet records',()=>{
 const h=lab();h.run(`startLocalProject(state.p,'city');startLocalProject(state.p,'city');stxSDAllocatePlanet(state.p);tickLocalProject(state.p,20);stxLPMobilize(state.p,6);state.id=state.p.id;state.before=JSON.stringify({pop:state.p.pop,cap:state.p.capacity,modes:state.p.factoryModes,vessels:state.p.reserveVessels,crew:state.p.stock.trained,projects:state.p.stxLocalProjects.map(q=>q.progress),fleet:{id:state.fleets[0].id,name:state.fleets[0].name,strength:state.fleets[0].strength,location:state.fleets[0].location,vesselCount:state.fleets[0].vesselCount,personnel:state.fleets[0].personnel,sources:state.fleets[0].mobilizationSources,commissionCredits:state.fleets[0].commissionCredits}});saveGame(false);loadGame();state.p=state.planets.find(p=>p.id===state.id)`);
 assert.equal(h.run('JSON.stringify({pop:state.p.pop,cap:state.p.capacity,modes:state.p.factoryModes,vessels:state.p.reserveVessels,crew:state.p.stock.trained,projects:state.p.stxLocalProjects.map(q=>q.progress),fleet:{id:state.fleets[0].id,name:state.fleets[0].name,strength:state.fleets[0].strength,location:state.fleets[0].location,vesselCount:state.fleets[0].vesselCount,personnel:state.fleets[0].personnel,sources:state.fleets[0].mobilizationSources,commissionCredits:state.fleets[0].commissionCredits}})'),h.run('state.before'));assert.equal(h.run('state.p.localProject===state.p.stxLocalProjects[0]'),true);assert.equal(h.run('stxSDDescriptors(state.p).filter(d=>d.kind==="local").length'),2);
});
...
test('AI worlds use population factories and reserve-limited commissioning',()=>{
 const h=lab();h.run('state.ai=owned(1)[0];empire(1).credits=100;state.ai.infra.factory=2;state.ai.infra.shipyard=1;stxLPEnsure(state.ai);state.ai.factoryModes=["war","components"];state.ai.reserveVessels=0;state.ai.stock.trained=.02;state.ai.pop=.4;state.ai.capacity=1;state.aiPop=state.ai.pop');assert.equal(h.run('stxQueueFleetCommission(state.ai)'),false);h.run('tickPlanet(state.ai,10)');assert.ok(h.run('state.ai.reserveVessels')>0);assert.ok(h.run('state.ai.pop')>h.run('state.aiPop'));h.run('state.ai.reserveVessels=15');assert.equal(h.run('stxQueueFleetCommission(state.ai)'),true);near(h.run('state.ai.reserveVessels'),0);near(h.run('empire(1).credits'),70);
});
