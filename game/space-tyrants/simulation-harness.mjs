...
export function createSimulation({seed=937145,excludePatches=[],canvasFactory=null,documentFactory=null}={}){
  ...
  const make=(tag='div',id='')=>{
    const native=canvasFactory&&(tag==='canvas'||id==='galaxy')?canvasFactory(1200,800):null;
    const nativePaint=native&&new Proxy(native.getContext('2d'),{
      get(t,k){if(k==='drawImage')return (source,...args)=>t.drawImage(source.__canvas||source,...args);const v=t[k];return typeof v==='function'?v.bind(t):v},
    ...
    const node={tagName:tag.toUpperCase(),id,hidden:true,innerHTML:'',textContent:'',value:'',children:[],dataset:{},attributes:{},clientWidth:1200,clientHeight:800,width:1200,height:800,scrollTop:0,scrollHeight:0,offsetHeight:0,isConnected:true,
      ...
      dispatchEvent(event){for(const fn of listeners.get(event.type)||[])fn(event);return !event.defaultPrevented},
      setAttribute(k,v){this.attributes[k]=v},getAttribute(k){return this.attributes[k]},removeAttribute(){},getContext:()=>nativePaint||paint,getBoundingClientRect:()=>({left:0,top:0,right:1200,bottom:800,width:1200,height:800}),
    ...
    if(native){node.__canvas=native;for(const key of ['width','height'])Object.defineProperty(node,key,{get:()=>native[key],set:value=>native[key]=value})}
  ...
  const doc=documentFactory?documentFactory(core,paint):{getElementById:id=>elements.get(id)||null,createElement:tag=>make(tag),head:make('head'),body:make('body'),documentElement:make('html'),querySelector:()=>null,querySelectorAll:()=>[],addEventListener(){}};
  ...
  const patches=paths.filter(p=>!excludePatches.includes(p)).map(p=>readFileSync(new URL(p,import.meta.url),'utf8')).join('\n');
