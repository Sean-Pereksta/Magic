...
function clampRating(v){return Math.max(1,Math.min(100,Math.round(v)))}
...
function normalizeReceiver(p){if(!p||typeof p!=='object')return newReceiver(false);for(const k of ['speed','cutting','turning','evasion','catching'])p[k]=clampRating(Number(p[k])||50);p.strength=Number.isFinite(Number(p.strength))?clampRating(Number(p.strength)):fallbackReceiverStrength(p);p.trainings=Math.max(0,Math.min(5,Number(p.trainings)||0));p.name=String(p.name||'').slice(0,80)||`${firstNames[randInt(0,firstNames.length-1)]} ${lastNames[randInt(0,lastNames.length-1)]}`;p.archetype=String(p.archetype||'Prospect').slice(0,40);p.id=p.id||`wr_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,8)}`;p.price=Math.max(100,Number(p.price)||200);p.appearance=normalizeAppearance(p.appearance,true);P.migratePlayer(p);return p}
...
function setupPlay(increment=false){
  ...
  for(let i=0;i<4;i++){
    ...
    const speed=THREE.MathUtils.lerp(6.72,7.92,profile.speed/100)+[.06,-.03,.02,.08][i],catchReach=THREE.MathUtils.lerp(.94,1.08,profile.catching/100);for(const arm of(mesh.userData.arms||[]))arm.scale.y=catchReach;
...
function makeDefender(mesh,kind,target,skill){
  ...
  return{mesh,kind,target,strength,fakeUntil:0,velocity:new THREE.Vector3(),impactVel:new THREE.Vector3(),heading:new THREE.Vector3(0,0,-1),reaction:Math.max(.06,reaction),turnRate,accel,maxSpeed,commit:0,lastDesired:new THREE.Vector3(),shoveCooldown:0,shoveSlow:0,stagger:0,shoveAnim:0,jumpY:0,jumpVel:0,jumpCooldown:0,swatPose:0,ballSeen:false,plantPose:0,runPhase:Math.random()*Math.PI*2,runIntensity:0};
}
...
function updateJump(a,dt,shouldJump=false){
  a.jumpCooldown=Math.max(0,(a.jumpCooldown||0)-dt);if(shouldJump&&a.jumpY<.03&&a.jumpCooldown<=0){a.jumpVel=a.profile?P.traits(a.profile).jumpVelocity:3.25;a.jumpCooldown=.75}
...
function actorStrength(a){return THREE.MathUtils.clamp(a?.profile?(a.profile.strength||50)/100+P.traits(a.profile).bodyBonus:(a?.strength||50)/100,.01,1)}
...
function contestedStrengthBonus(receiver,defender,distance){if(!receiver||!defender||distance>=1.45)return 0;const rs=actorStrength(receiver),ds=actorStrength(defender),closeness=distance<.85?1:.62;return THREE.MathUtils.clamp(((rs-ds)*.24+(rs-.5)*.06+(receiver.profile?Math.max(0,receiver.profile.size-50)*.001:0))*closeness,-.11,.10)}
...
function checkEmergencyDiveCatch(){
  ...
  for(const r of receivers){
    ...
    const catchRating=(r.profile?.catching||50)/100,normalRadius=THREE.MathUtils.lerp(.84,.96,catchRating),normalDist=receiverNormalCatchDistance(r);
  ...
  const contestInfo=nearestOpponentInfo(best,false),contest=contestInfo.dist,strengthFight=contestedStrengthBonus(best,contestInfo.player,contest)*.68,stretch=THREE.MathUtils.clamp(bestReach/(best.rescueRadius||1.62),0,1),catchBonus=((best.profile?.catching||50)-50)/50*.055,heightDifficulty=THREE.MathUtils.clamp((ball.position.y-.18)/.90,0,1),contestPenalty=contest<.85?.20:contest<1.45?.09:0,comebackPenalty=best.comebackActive?(best.underthrowDifficulty||.08)*.45:0;
  // Intentionally much lower than the normal catch formula. STR helps only if a defender is actually contesting the dive.
  const catchChance=THREE.MathUtils.clamp(.66+(best.profile.athleticism-50)*.0012-stretch*.24-heightDifficulty*.05+spiralQuality*.08+catchBonus+strengthFight-contestPenalty-comebackPenalty-(best.stagger>0?.07:0),.22,.76);
...
function updateBallCarrier(r,dt){
  ...
  const evade=new THREE.Vector3();for(const d of defenders){const away=pos.clone().sub(d.mesh.position);away.y=0;const dist=away.length();if(dist>0&&dist<9){away.normalize();const ahead=d.mesh.position.z<pos.z?1.15:.72;evade.addScaledVector(away,(9-dist)/9*ahead)}}if(Math.abs(pos.x)>21)evade.x+=-Math.sign(pos.x)*1.6;const evadeRating=(r.profile?.evasion||50)/100;desired.addScaledVector(evade,THREE.MathUtils.lerp(.70,1.18,evadeRating)).normalize();
...
function updateRunAfterCatch(dt,now){
  ...
  for(const d of defenders){const dx=d.mesh.position.x-ballCarrier.mesh.position.x,dz=d.mesh.position.z-ballCarrier.mesh.position.z;const evadeRating=(ballCarrier.profile?.evasion||50)/100,tackleRadius=THREE.MathUtils.lerp(1.04,.86,evadeRating);if(d.fakeUntil<=gameTime&&Math.hypot(dx,dz)<tackleRadius){triggerTackle(d);return}}solvePlayerCollisions(dt);attachBallToCarrier();
}
...
function tryJuke(r,manual=false){
  ...
  const lateral=new THREE.Vector3(side,0,0);r.impactVel.addScaledVector(lateral,1.7+r.profile.tricks*.012);
  for(const d of candidates){
    const chance=THREE.MathUtils.clamp(traits.jukeChance-currentSkill()*.20,.12,.82);
    if(Math.random()<chance){d.fakeUntil=gameTime+260+r.profile.tricks*2.4;d.fakeTarget=r.mesh.position.clone().add(new THREE.Vector3(-side*3,0,-1));d.plantPose=1;d.velocity.multiplyScalar(.65)}
  }
...
function updateTricks(r,dt,carrier){
  ...
  if(!ballLive&&(carrier||playState==='live')&&Math.random()<dt*(.5+r.profile.tricks*.009))tryJuke(r);
}
