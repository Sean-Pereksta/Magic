...
function context(){const c=vm.createContext({THREE,P,ballLive:true,ball:{position:new THREE.Vector3()},ballVel:new THREE.Vector3(),gameTime:1000,defenders:[],ballCarrier:null,currentSkill:()=>.8,flashResult:()=>{},Math:Object.create(Math)});for(const name of ['receiverHandTarget','trackReceiverHands','animatePlayerContact','tryJuke','defenderTarget'])vm.runInContext(extract(name),c);return c;}
